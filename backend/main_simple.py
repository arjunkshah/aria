...
user_emails = {}  # Store user emails for notifications
...
class AutoGenerationService:
    ...
    def start_monitoring(self, project_id: str, config: Dict):
    ...
    async def _monitor_project(self, project_id: str):
        ...
        while project_id in self.tasks and self.tasks[project_id]['running']:
            try:
                ...
                await asyncio.sleep(self.tasks[project_id]['config'].get('check_interval', 300))
            except Exception as e:
    ...
    async def _send_notification(self, project_id: str, notification: Dict):
        ...
        # Send email notification if user email is configured
        if project_id in user_emails:
            user_email = user_emails[project_id]
            project_name = projects.get(project_id, {}).get('name', 'Unknown Project')
            
            # Send email notification
            if notification['type'] == 'success' and 'changelog' in notification['title'].lower():
                # Extract repository name from notification message
                message = notification['message']
                if 'for' in message:
                    repo_name = message.split('for ')[-1].split(' with')[0]
                    version = message.split('Generated ')[1].split(' for')[0]
                    pr_count = int(message.split('with ')[-1].split(' changes')[0])
                    
                    # Send changelog email
                    email_service.send_changelog_notification(
                        user_email, repo_name, version, 
                        [f"Auto-generated changelog for {repo_name}"], pr_count
                    )
            else:
                # Send general notification email
                email_service.send_project_notification(
                    user_email, project_name, "Repository", notification['title']
                )
...
async def create_project(request: Dict):
    ...
    projects[project_id] = {
        ...
        'user_email': request.get('user_email', ''),  # Add user email
    ...
    if request.get('user_email'):
        user_emails[project_id] = request.get('user_email')
    
    print(f"Created project: {projects[project_id]['name']} (ID: {project_id})")
...
async def connect_repository(request: Dict):
    ...
    token = token or projects[project_id]['github_token']
    ...
    try:
        ...
        connected_repos[project_id][repo_id] = {
            'repo_url': repo_url,
            'owner': owner,
            'name': repo_name,
        ...
        print(f"Connected repository: {repo_id} to project {project_id}")
...
async def start_auto_generation(request: Dict):
    ...
    auto_generation_service.start_monitoring(project_id, config)
    
    return {"message": f"Auto-generation started for project {project_id}"}
...
# Email management
@app.post("/projects/{project_id}/email")
async def set_project_email(project_id: str, request: Dict):
    """Set email for project notifications"""
    if project_id not in projects:
        raise HTTPException(status_code=404, detail="Project not found")
    ...
    projects[project_id]['user_email'] = email
    user_emails[project_id] = email
    
    print(f"Set email for project {project_id}: {email}")
    return {"message": f"Email configured for project {project_id}"}

@app.get("/projects/{project_id}/email")
async def get_project_email(project_id: str):
    """Get email for project notifications"""
    if project_id not in projects:
        raise HTTPException(status_code=404, detail="Project not found")
    
    email = projects[project_id].get('user_email', '')
    return {"email": email}
...
async def test_email(request: Dict):
    ...
    # Send test email
    success = email_service.send_project_notification(
        email, "Test Project", "Test Repository", "Email Test"
    )
